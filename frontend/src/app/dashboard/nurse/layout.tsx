...
const navItems = [
  { label: "Dashboard", icon: LayoutDashboard, href: "/dashboard/nurse" },
  { label: "My Patients", icon: Users, href: "/dashboard/nurse/patients" },
  { label: "Notifications", icon: Bell, href: "/dashboard/nurse/notifications" },
  { label: "My Profile", icon: User, href: "/dashboard/nurse/profile" },
];
...
export default function NurseLayout({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (href: string) => {
    if (href === "/dashboard/nurse") return pathname === "/dashboard/nurse";
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-6 border-b border-slate-700/50">
        <Link href="/dashboard/nurse" className="flex items-center gap-3">
          ...
          {!collapsed && (
            ...
            >
              ...
              <span className="text-[10px] font-bold tracking-widest text-cyan-400/70 uppercase">Nurse Portal</span>
            </motion.div>
          )}
        </Link>
      </div>
        ...
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-4 sm:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg sm:text-xl font-extrabold text-slate-800">Nurse Dashboard</h1>
                <p className="text-xs text-slate-500 font-medium hidden sm:block">PQC Secure Hospital Management System</p>
              </div>
            </div>
