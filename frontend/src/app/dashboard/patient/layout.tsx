...
import { Activity, Bell, Calendar, ChevronLeft, FileHeart, FileText, Heart, LayoutDashboard, LogOut, Menu, Pill, ShieldCheck, ShieldOff, ShieldQuestion, Stethoscope, TestTubes, User, X } from "lucide-react";
...
export default function PatientLayout({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (href: string) => {
    if (href === "/dashboard/patient") return pathname === "/dashboard/patient";
        ...
        <Link href="/dashboard/patient" className="flex items-center gap-3">
          ...
          {!collapsed && (
            ...
            >
              ...
              <span className="text-[10px] font-bold tracking-widest text-cyan-400/70 uppercase">Patient Portal</span>
            </motion.div>
          )}
        </Link>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {active && !collapsed && (
                <motion.div
                  layoutId="patient-active-indicator"
        ...
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-4 sm:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg sm:text-xl font-extrabold text-slate-800">Patient Dashboard</h1>
