...
// Mirrors the backend NotificationItem. An earlier version read `message`,
// `time` and `read`, none of which the API returns.
  ...
  useEffect(() => {
    const fetchNotifs = async () => {
      try {
        ...
        console.error(error);
        // No invented notifications, especially ones asserting that keys were
        // rotated or a report was signed — those describe security events.
        setNotifications([]);
            ...
            <AnimatePresence>
              {notifications.map((notification: LabTechNotifItem) => (
                <motion.div
                  ...
                  className={`p-4 flex gap-4 transition-colors ${notification.read_at ? 'bg-white' : 'bg-cyan-50/30'}`}
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${notification.read_at ? 'bg-slate-100' : 'bg-white border border-cyan-100 shadow-sm'}`}>
                    {getIcon(notification.notification_type || "system")}

                  </div>
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      <h3 className={`text-sm font-bold ${notification.read_at ? 'text-slate-700' : 'text-slate-900'}`}>
                        {notification.title}
                      </h3>
                    ...
                    <p className={`text-sm ${notification.read_at ? 'text-slate-500' : 'text-slate-600 font-medium'}`}>
                      {notification.body}
                    </p>
                  </div>
